...
public class TrabajadorController {
	...
	private String obtenerUsername(Trabajador trabajador) {
		...
		String usarname = trabajador.getNombres().substring(0, 1) + trabajador.getApePater()
				+ trabajador.getApeMater().substring(0, 1) + sdia;
		Usuario buscar_repetido = serviceUsuario.findbyUsername(usarname);
		if (buscar_repetido != null) {
			usarname = trabajador.getNombres().substring(0, 1) + trabajador.getApePater()
					+ trabajador.getApeMater().substring(0, 2) + sdia;
			Usuario buscar_repetido2 = serviceUsuario.findbyUsername(usarname);
			if (buscar_repetido2 != null) {
				usarname = trabajador.getNombres().substring(0, 2) + trabajador.getApePater()
						+ trabajador.getApeMater().substring(0, 2) + sdia;
				Usuario buscar_repetido3 = serviceUsuario.findbyUsername(usarname);
	...
	/*@Autowired
	AfpService serviceAfp;

	@Autowired
	ParametroService serviceParametro;

	@Autowired
	VacacionesService serviceVacaciones;
	...
	public ResponseWrapper liquidar(@RequestBody ContratoDTO contratoDto, BindingResult result) throws Exception {

		if (result.hasErrors()) {
			List<String> errors = result.getFieldErrors().stream().map(err -> {
				return err.getDefaultMessage();
			}).collect(Collectors.toList());
			throw new ExceptionResponse(new Date(), this.getClass().getSimpleName() + "/liquidar",
					"Error en la validacion: Lista de Errores:" + errors.toString(), contratoDto);
		}
		try {
			ResponseWrapper response = new ResponseWrapper();
			...
			double cts = calculoCts(contratoDto);
			double vacaTruncas = calculoVacaTrunca(contratoDto);

			response.setEstado(Constantes.valTransaccionOk);
			return response;
		} catch (Exception e) {
			System.out.println(this.getClass().getSimpleName() + " liquidarTrabajador. ERROR : " + e.getMessage());
			throw new ExceptionResponse(new Date(), this.getClass().getSimpleName() + "/liquidar",
					e.getStackTrace()[0].getFileName() + " => " + e.getStackTrace()[0].getMethodName() + " => "
							+ e.getClass() + " => message: " + e.getMessage() + "=> linea nro: "
							+ e.getStackTrace()[0].getLineNumber(),
	...
	private double calculoVacaTrunca(ContratoDTO contratoDto) {
		...
		Contrato contrato = contratoDto.getContrato();
		Trabajador trabajador = contrato.getTrabajador();
		Afp afp = trabajador.getAfp();
		// PdoMes mes = contratoDto.getPdoMes();

		double sueldoBase = contrato.getSueldoBase();
		Parametro interesCts = serviceParametro.buscarPorCodigoAndEmpresa(Constantes.CODPORCINCTS,
				trabajador.getEmpresa());
		double inteCts = Double.parseDouble(interesCts.getValor());

		Parametro diasComputables = serviceParametro.buscarPorCodigoAndEmpresa(Constantes.CODDIASCOMPTBASE, null);
		int diasComp = Integer.parseInt(diasComputables.getValor());

		int cantidadDias = 0;
		List<Vacaciones> listVacacionesLibres = serviceVacaciones.buscarPorTrabyEstado(trabajador, 1);
		for (Vacaciones vacaciones : listVacacionesLibres) {
			cantidadDias += 15;
		}

		List<Vacaciones> listVacacionesParcial = serviceVacaciones.buscarPorTrabyEstado(trabajador, 2);
		for (Vacaciones vacaciones : listVacacionesParcial) {
			cantidadDias += 15 - (vacaciones.getDiasTomados() + vacaciones.getDiasVendidos());
		}
		...
		double totalAfpOnp = 0.0;
		if (afp.getIdAfp() == 1) {
			double porcentaje = afp.getApoOblFndPen();
			totalAfpOnp = sueldoBase * porcentaje;
		} else {
			double porcentaje = afp.getApoOblFndPen() + afp.getComSobFlu() + afp.getPrimaSeguro();
			totalAfpOnp = sueldoBase * porcentaje;
		}
	...
	private Planilla crearObjetoPlanilla(ContratoDTO contratoDTO) {
		...
		Contrato contrato = contratoDTO.getContrato();
		PdoAno pdoAno = contratoDTO.getPdoAno();
		PdoMes pdoMes = contratoDTO.getPdoMes();
		Empresa empresa = contrato.getTrabajador().getEmpresa();
		Parametro tipoTardanza = service_para.buscarTardanzaPorEmpresa(empresa);
		Parametro tolerancia = obtenerTolerancia(empresa);
		int toleranciaMinutos = Integer.parseInt(tolerancia.getValor());
		Parametro rango = service_para.buscarRangoPorEmpresa(empresa);
		...
		double deudaAdelantoSueldo = pagarDeuda(contratoDTO);
	...
	private double[] obtenerHorasExtras(List<Asistencia> lsasistencia, Horario horario) {
		...
		for (Asistencia asistencia : lsasistencia) {
			...
			if (horarioHora == asistenciaHora) { // debe superar 5min
	...
	public double pagarDeuda(ContratoDTO contratoDTO) {
		...
		Trabajador trab = contratoDTO.getContrato().getTrabajador();
		PdoAno pdoAno = contratoDTO.getPdoAno();
		PdoMes pdoMes = contratoDTO.getPdoMes();
